...
//ingresar_02direccionproveedor1
	...
	)
	{
		...
		/*if( $this->verificar_rfc($conn,$RFC_PROVEEDOR) ==''){
			$idwebc = $this->ingresar_usuario($conn,$NOMBRE_COMERCIAL);
...
//ENVIARPAGOprovee
		...
		$detalleActualizacion = 'se actualizó el registro de PAGO A PROVEEDOR-A.';
		...
		$this->registrar_bitacora(
			...
			$ultimo_id,
			'INGRESO',
			'se ingresó un nuevo registro de PAGO A PROVEEDOR-A.',
			$usuarioBitacora,
			''
		);
...
/* DATOS BANCARIOS 1 */ 
	...
	$P_NUMERO_DE_SUCURSAL_1 , $P_NUMERO_IBAN_1 , $P_NUMERO_CUENTA_SWIFT_1,$FOTO_ESTADO_PROVEE,$ULTIMA_CARGA_DATOBANCA, $ENVIARRdatosbancario1p,$IPdatosbancario1p ){
		...
		mysqli_query($conn,$var2) or die('P160'.mysqli_error($conn));
		$nuevo_id_datos = mysqli_insert_id($conn);

		$this->registrar_bitacora_sb('crear', 'Se creó un registro en 02DATOSBANCARIOS1', $nuevo_id_datos, '02DATOSBANCARIOS1');
...
'".$NoIdentificacionConcepto."'
	...
	//hiddensubefactura
		...
		$ultimo_id ='';		
		$ultimo_id = mysqli_insert_id($conn);
		$this->registrar_bitacora_sb('crear', 'Se creó un registro en 02SUBETUFACTURA', $ultimo_id, '02SUBETUFACTURA');
...
///////////////////////////// /////////////////////////
    ...
    mysqli_query($conn,$var2) or die('P160'.mysqli_error($conn));
	   $nuevo_id_notas = mysqli_insert_id($conn);

    $this->registrar_bitacora_sb('crear', 'Se creó un registro en 02NOTAS', $nuevo_id_notas, '02NOTAS');
...
/* DATOS BANCARIOS 1 */ 
	...
	public function documentofiscal ($DOCUMENTO_LEGAL ,$ADJUNTAR_DOCUMENTO_LEGAL, $ADJUNTAR_DOCUMENTO_OBSERVACIONES , $FECHA_ULTIMA_DOCUMEN , $validaDOCUMENTOSFISCAL,$IPdocumentosfiscales,$ENVIAFISCAL){
		...
		mysqli_query($conn,$var2) or die('P160'.mysqli_error($conn));
			$nuevo_id_fiscal = mysqli_insert_id($conn);

		$this->registrar_bitacora_sb('crear', 'Se creó un registro en 02DOCUMENTOSFISCALES', $nuevo_id_fiscal, '02DOCUMENTOSFISCALES');
