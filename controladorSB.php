 ...
 * seguido ÚNICAMENTE de números, sin letras, espacios ni caracteres adicionales.
 * Ejemplos válidos: EPC123, INN45, EVE1
 * Ejemplos inválidos: EPC, EPCA123, EPC12A, EPC 123 */
if(!function_exists('formatoNumeroEventoValidoSB')){
 
	function formatoNumeroEventoValidoSB($numeroEvento){
 
		$numeroEvento = trim((string)$numeroEvento);
 
		return (bool) preg_match('/^(EPC|INN|EVE)[0-9]+$/i', $numeroEvento);
...
if(!formatoNumeroEventoValidoSB($NUMERO_EVENTO)){
        echo "<P style='color:red; font-size:23px;'>EL NÚMERO DE EVENTO DEBE SER LAS INICIALES DE LA EMPRESA (EPC, INN O EVE) SEGUIDAS ÚNICAMENTE DE NÚMEROS, SIN LETRAS, ESPACIOS NI CARACTERES ADICIONALES. EJEMPLO: EPC123.</p>";
			exit;
...
elseif($NOMBRE_COMERCIAL == "" or $NUMERO_EVENTO == "" or $NOMBRE_DEL_EJECUTIVO == "" or $MONTO_TOTAL_COTIZACION_ADEUDO == ""){
	...
	echo $SUBEFACTURA->lectorxmlX ($NUMERO_CONSECUTIVO_PROVEE , $NOMBRE_COMERCIAL , $RAZON_SOCIAL ,$VIATICOSOPRO, $RFC_PROVEEDOR , $NUMERO_EVENTO , $NOMBRE_EVENTO , $CONCEPTO_PROVEE , $MONTO_TOTAL_COTIZACION_ADEUDO , $MONTO_DEPOSITAR , $MONTO_PROPINA , $MONTO_FACTURA , $TIPO_DE_MONEDA ,$PFORMADE_PAGO, $STATUS_DE_PAGO , $NOMBRE_DEL_EJECUTIVO , $OBSERVACIONES_1 ,$FECHA_DE_LLENADO, $ADJUNTAR_FACTURA_XML , $ADJUNTAR_FACTURA_PDF, $ADJUNTAR_COTIZACION11, $CONPROBANTE_TRANSFERENCIA, $ADJUNTAR_ARCHIVO_1,$IMPUESTO_HOSPEDAJE, $MONTO_DEPOSITADO,$PENDIENTE_PAGO,$IVA,$NOMBRE_DEL_AYUDO,$TImpuestosRetenidosIVA,$TImpuestosRetenidosISR,$descuentos,$hiddensubefactura, $ENVIARRSB1p, $IPSB1p);
	//include_once (__ROOT1__."/includes/crea_funciones.php");
